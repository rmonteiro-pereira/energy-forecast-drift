...
| Retrain verdict + four signal chips | `drift.json` → `verdict`, `drift.*.severity` |
| Headline stats | `monitor.json`, `pipeline.json` |
| Forecast vs actual | `forecast.json` → `history` (scored) and `forward` (live) |
