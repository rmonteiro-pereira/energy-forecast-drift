...
Two PNGs, 397 KB total. Both are regenerated by `python -m pipeline.daily`, and both carry
a rotated `SYNTHETIC FIXTURE — NOT REAL DATA` watermark drawn by
`pipeline/plots.py::_stamp_if_synthetic` whenever `is_real` is false — which is now, and
which is why the watermark is visible in the committed copies.
