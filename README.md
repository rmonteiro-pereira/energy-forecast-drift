...
| | Status | Notes |
|---|---|---|
| **The code** | ✅ real, complete, tested | 379 Python tests + 4 dashboard tests, no network. Every path below runs today, and [`docs/REPRODUCE.md`](docs/REPRODUCE.md) has the transcripts. The tests are themselves measured — see [mutation testing](docs/MUTATION-TESTING.md). |
...
uv run pytest -v               # 379 tests, no network
...
tests/      379 Python tests: idempotency, leakage (backtest *and* features),
