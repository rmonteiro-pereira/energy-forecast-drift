...
uv run pytest -v               # 384 tests, no network
...
tests/      384 Python tests: idempotency, leakage (backtest *and* features),
...
| **next** | regenerate `metrics/baseline.json` on real load. It is the last synthetic artifact, nothing reads it, and the real numbers already exist inside `model.json`, so this is tidying a visible gap rather than closing a claim |
