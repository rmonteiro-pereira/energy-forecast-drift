...
> [!NOTE]
> # Every number here is measured on real PJM load. Nothing committed is synthetic anymore.
>
...
>
> **Real, from the EIA API:** 17,524+ hourly PJM demand records from
> 2024-08-01 onward with **no missing hours**, and everything derived from
> them: [`metrics/forecast.json`](metrics/forecast.json),
> [`monitor.json`](metrics/monitor.json), [`drift.json`](metrics/drift.json),
> [`pipeline.json`](metrics/pipeline.json),
> [`model.json`](metrics/model.json) and
> [`baseline.json`](metrics/baseline.json), each carrying `"is_real": true`.
> Both committed PNGs are real too, and are no longer watermarked because they
...
> That is **77% less absolute error**, winning **24 of 24 horizons**. Removing the
...
> the gate, and served.
>
> **The fixture era is over, and it is documented rather than erased:**
> `baseline.json`, the last artifact that predated the key, was regenerated on
> the real lake on 2026-08-05. The fixture-era outputs are kept below in
> collapsed blocks, labelled as history — they demonstrate that the plumbing
> ran before the data landed, and nothing in them is a result.
>
...
measured and beats its baseline at every horizon. It is the loop around it, and
the loop both exists and runs on its own: one command chains ingest → features →
score → rolling-MAE monitor → drift → artifacts, `daily.yml` calls exactly that
command, and **it is scheduled** at 06:20 UTC, every day, publishing by pull
request. Drift now accumulates in public week after week and can be
pointed at.
...
| | Status | Notes |
|---|---|---|
| **The code** | ✅ real, complete, tested | 511 Python tests + 7 dashboard tests, no network. Every path below runs today, and [`docs/REPRODUCE.md`](docs/REPRODUCE.md) has the transcripts. The tests are themselves measured — see [mutation testing](docs/MUTATION-TESTING.md). |
...
| **`baseline.json`** | ✅ **real** | Regenerated on the real lake on 2026-08-05 (`regenerate-baseline.yml`), retiring the last `is_real: false` artifact. The comparison quoted above still comes from `model.json → metrics.baseline` — the same baseline on the same folds as the model — and the standalone artifact now agrees with it instead of describing a fixture. |
...
uv run pytest -v               # 511 tests, no network
...
**Read this before the table.** Every cell below was computed from
`models/fixtures.py` — a seeded curve, not the EIA — in the era before the key
landed. It is kept as the record that the backtest ran, was scored per horizon
and produced a well-formed artifact **before** any real data existed. It is
**not** a measurement of anything, and the figures are deliberately left
unbolded so that nothing here reads as a headline. The committed
[`metrics/baseline.json`](metrics/baseline.json) has since been regenerated on
real load and no longer matches this table — the current numbers live in
[`metrics/baseline_table.md`](metrics/baseline_table.md).
...
*(fixture-era output, as committed before 2026-08-05 — the file has since been
regenerated on real load and carries `"is_real": true`)*
...
**The delta below was the most quotable-looking number in this repository and
the most misleading.** It is fixture against fixture, kept as history — the
measured comparison on real load (2.72% vs 11.73% MAPE) lives in the banner
and in [`metrics/model.json`](metrics/model.json), which now carries
`"is_real": true`. A synthetic curve is easier to
...
*(fixture-era output, as committed before the 2026-08-02 train on real demand —
`metrics/model.json` has carried `"is_real": true` since)*
...
tests/      511 Python tests: idempotency, leakage (backtest *and* features),
...
| **done, 2026-08-05** | `regenerate-baseline.yml` dispatched: `metrics/baseline.json` regenerated on the real lake, retiring the last `is_real: false` artifact. The banner dropped from `[!WARNING]` to `[!NOTE]` in the same stroke — the alert level is derived from the artifacts by test, and with nothing synthetic left, an alarm would overstate |
